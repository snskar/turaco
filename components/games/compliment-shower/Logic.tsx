...
const DROP_SPEED_RATIO = 0.01; // portion of screen height per frame
const JAR_WIDTH_RATIO = 0.15; // portion of screen width
const DROP_SIZE_RATIO = 0.04; // portion of screen width
...
export default function ComplimentShower() {
        ...
        return prev
          ...
          .filter((drop) => {
            ...
            if (caught) {
              ...
              complimentTimeoutRef.current = setTimeout(() => setCompliment(null), 2000);
            }
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    setJarXRatio(Math.min(1 - JAR_WIDTH_RATIO, Math.max(0, ratio - JAR_WIDTH_RATIO / 2)));
  };
  ...
  return (
    <div
      className="relative w-full h-screen bg-blue-100 overflow-hidden touch-none"
    ...
    >
      ...
      <AnimatePresence>
        {compliment && (
          <motion.div
            className="absolute top-10 left-1/2 transform -translate-x-1/2 text-xl font-semibold text-pink-600"
      ...
      {drops.map((drop) => (
        <motion.div
          ...
          style={{
            ...
            width: dropSize,
            height: dropSize,
            backgroundImage: "url(/assets/drop.png)",
      ...
      <motion.div
        className="absolute bottom-4 h-12 bg-purple-500 rounded-full"
        style={{
          width: jarWidth,
