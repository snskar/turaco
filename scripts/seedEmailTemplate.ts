/**
 * Script to seed the email template in the database
 * Run this in production to ensure the template exists:
 * npx tsx scripts/seedEmailTemplate.ts
 */
...
async function main() {
  try {
    console.log('🔍 Checking for existing email template...');
    ...
    if (existing) {
      console.log('✅ Email template already exists:', existing.id);
      ...
      const updated = await prisma.emailTemplate.update({
        ...
        data: {
          ...
          bodyText: DEFAULT_TEMPLATE.bodyText,
          bodyHtml: DEFAULT_TEMPLATE.bodyHtml,
          isActive: DEFAULT_TEMPLATE.isActive,
        },
      });
